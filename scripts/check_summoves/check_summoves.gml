...
summoves2 = instance_number(hypercube) //+ future_summoves
//1 O - O
//    Θ
...
for(i=0;i<=global.board_rows-1;i++)
    ...
    for(j=0;j<=board_xsize-1;j++)
        {   
            ...
            if gemboard[i-1,j-1] = gemboard[i-1,j+1] && (gemboard[i-1,j-1] >= 0 && gemboard[i-1,j-1] <=6)
                {
                    ...
                    if gemboard[i-1,j] != -1 //now check if the clickable gem is also not void
                        {
                            if !gems_fallen[i-1,j].amLocked //and if its not void, check if its locked
                            {
                                if gemboard[i,j] = skin && !gems_fallen[i,j].amLocked//all good? last thing to do is check if Θ is same and not locked
                                    {
            ...
            //2
            if (i <=global.board_rows-2) && (j >= 1 && j <= board_xsize-2)
            {
            //first check if the two other gems are the same (and not coal/void)
            if gemboard[i+1,j-1] = gemboard[i+1,j+1] && (gemboard[i+1,j-1] >= 0 && gemboard[i+1,j-1] <=6)
                {
            ...
            //3
            if (i >=1 && i <=global.board_rows-2) && (j >= 1)
            {
            //first check if the two other gems are the same (and not coal/void)
            if gemboard[i-1,j-1] = gemboard[i+1,j-1] && (gemboard[i-1,j-1] >= 0 && gemboard[i-1,j-1] <=6)
                {
                    ...
                    if gemboard[i,j-1] != -1 //now check if the clickable gem is also not void
                        {
                            if !gems_fallen[i,j-1].amLocked //and if its not void, check if its locked
                            {
                                if gemboard[i,j] = skin && !gems_fallen[i,j].amLocked//all good? last thing to do is check if Θ is same and not locked
                                    { 
										i_gemtouse[summoves2] = i //the i position of the bot gem to use
            ...
            //4
            if (i >=1 && i <=global.board_rows-2) && (j <= board_xsize-2)
            {
            //first check if the two other gems are the same (and not coal/void)
            skin = gemboard[i-1,j+1]
            ...
            //8
            if (i <=global.board_rows-3) && (j <= board_xsize-2)
            {
            //first check if the two other gems are the same (and not coal/void)
            skin = gemboard[i+1,j+1]
            if skin = gemboard[i+2,j+1] && (skin >= 0 && skin <=6)
            ...
            //10
            if (i <=global.board_rows-2) && (j <= board_xsize-3)
            {
            //first check if the two other gems are the same (and not coal/void)
            skin = gemboard[i+1,j+1]
            if skin = gemboard[i+1,j+2] && (skin >= 0 && skin <=6)
            ...
            //13
            if (i <= global.board_rows-4) && board_ysize > 3
            {
            //first check if the two other gems are the same (and not coal/void)
            skin = gemboard[i+2,j]
