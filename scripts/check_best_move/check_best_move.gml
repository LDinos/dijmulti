...
var best = 0
whattoshow = best_move[0] //debug
for(i=1;i<=argument0.summoves2-1;i++)
{
	if best_move[i] > best_move[i-1] //if it is bigger (example: I am a 4gem match, much better than the 3gem one.)
	{
		best = i
		whattoshow = best_move[i] //debug
	}
	else if (best_move[i] = best_move[i-1]) && (room = rm_avalanche)//if they are the same (example: both of them are 3 gem matches)
	{
		if best_I[i] > best_I[i-1] //find which one is in the lowest height (so cascades will be more possible)
		{
			best = i
			whattoshow = best_move[i] //debug
		}
